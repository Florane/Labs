...
void inputManual(Size size) // input manually
{
    int num[SIZE][SIZE];
...
void inputRandom(Size size) //input random from begin to end
{
    srand(time(0));
...
void inputFormula(Size size) //input through preset formula
{
    int num[SIZE][SIZE];
    // int** num = (int**) malloc(size.x*sizeof(int*));
    // for(int i = 0;i < size.x;i++)
...
{
    printf_s("Select: \n1: Enter manually\n2: Enter random\n3: Enter from formula\n4: Enter from file\n5: Enter from binary file\n");
    char c;
    do {
        c = _getch();
    } while(c<'1'||c>'5');
    ...
    switch(c)
    {
        case '1': inputManual(size); break;
        case '2': inputRandom(size); break;
        case '3': inputFormula(size); break;
        case '4': inputFromFile(); break;
        case '5': inputFromBinary(); break;
    }
}
